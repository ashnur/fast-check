    ...
    try {
        for (var _b = __values(Object.keys(recordModel)), _c = _b.next(); !_c.done; _c = _b.next()) {
            ...
            if (constraints.required && constraints.required.includes(k)) {
                ...
                updatedRecordModel[k] = OptionArbitrary_1.option(recordModel[k].map(function (v) { return ({ value: v }); }), constraints.frequencies[k]);
            }
            else {
                updatedRecordModel[k] = OptionArbitrary_1.option(recordModel[k].map(function (v) { return ({ value: v }); }), 1);
            }
        }
    }
